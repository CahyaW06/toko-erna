...
<div class="col-lg-4">
    <div class="card">
        <div class="card-body">
          ...
          <form class="forms-sample" action="{{ route('log.keuangan.update', ['keuangan' => $log->id]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="form-group">
              ...
              <div class="d-flex flex-column">
                <select class="form-control form-control-sm text-black" name="status" id="status">
                  <option @if($log->status == "Laku") selected @endif value="1">Laku</option>
                  <option @if($log->status == "Ganti Rugi") selected @endif value="2">Ganti Rugi</option>
                </select>
              </div>
            </div>
            <div class="form-group">
